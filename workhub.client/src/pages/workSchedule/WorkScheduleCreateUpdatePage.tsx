...
export const loader = wrapProtectedLoader(
  async ({ params }, { permissions }) => {
    ...
    if (!id) {
      ensurePermission(permissions, Permission.PermissionsWorkSchedulesCreate);
      return null;
    }

    ensurePermission(permissions, Permission.PermissionsWorkSchedulesEdit);

    const data = await wrapPromise(() =>
      workScheduleApi.workScheduleGetById({ id: +id })
    );
