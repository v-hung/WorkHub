...
export class UserApiRequestFactory extends BaseAPIRequestFactory {
    ...
    /**
     */
    public async userImportUserFromTimekeepingDevice(_options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // Path Params
        const localVarPath = '/api/users/sync-from-device';
...
export class UserApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to userImportUserFromTimekeepingDevice
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async userImportUserFromTimekeepingDeviceWithHttpInfo(response: ResponseContext): Promise<HttpInfo<BioStarSyncAllUsersResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
