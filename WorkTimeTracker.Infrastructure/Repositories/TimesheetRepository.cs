...
namespace WorkTimeTracker.Infrastructure.Repositories
{
	public class TimesheetRepository : ITimesheetRepository
	{
		...
		public async Task<List<D>> GetMonthlyTimesheets<D>(string userId, int month, int year) where D : class
		{
		...
		{
			return await _context.Timesheets.AsNoTracking()
				.Where(t => t.UserId == new Guid(userId) && EF.Functions.DateDiffDay(t.StartTime, DateTime.Today) == 0)
				.ProjectTo<D>(_mapper.ConfigurationProvider)
				.FirstOrDefaultAsync() ?? throw new BusinessException(HttpStatusCode.NotFound, _localizer["EntityNotFound"]);
		}

		public async Task<D> PerformCheckIn<D>(string userId) where D : class
		{
			Timesheet? timesheetDb = _context.Timesheets.FirstOrDefault(t => t.UserId == new Guid(userId) && EF.Functions.DateDiffDay(t.StartTime, DateTime.Today) == 0);
		...
		{
			Timesheet timesheet = _context.Timesheets.FirstOrDefault(t => t.UserId == new Guid(userId) && EF.Functions.DateDiffDay(t.StartTime, DateTime.Today) == 0) ?? throw new BusinessException(HttpStatusCode.NotFound, _localizer["EntityNotFound"]);
